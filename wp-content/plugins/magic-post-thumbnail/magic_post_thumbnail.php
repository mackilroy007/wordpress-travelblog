...
Copyright 2020 Magic Post Thumbnail (contact@magic-post-thumbnail.com)
...
<?php 
    ...
    public function MPT_remove_google_url( $str )
    {
        $pos = strpos( $str, '.gstatic.com/images' );
        if ( false === $pos ) {
            return $str;
        }
    }
    ...
    )
    {
        ...
        if ( $service == 'google_scraping' ) {
            /* New scraping method with regex */
            // Get all images from Google
            preg_match_all( '/,\\[\\"(.*)\\",\\d+?,\\d+?\\]/', $result['body'], $output_img_urls );
            $output_img_urls = $output_img_urls[1];
            // Remove first useless result
            unset( $output_img_urls[0] );
            $result_body['results'] = array_map( array( &$this, 'MPT_order_array_urls' ), array_filter( $output_img_urls, array( &$this, 'MPT_remove_google_url' ) ) );
            /* Old scraping method */
            /*
            $str = explode( '",168,300],["', $result['body'] );
            ...
            for( $a = 1; $a <7; $a++ ) {
            	...
            	if ( $str_image_url[0] != '' ) {
            		...
            		$path_parts = pathinfo( $str_image_url[0] );
            		
            		// Set to jpeg if extension is not defined
            		...
            		// Get current URL and block specific domain
            		$current_domain = parse_url($result_body['results'][$a]['url']);
            	}
            }
            
            for( $a = 1; $a < 7; $a++ ) {
            	if( empty( $result_body['results'][$a]['url'] ) ) {
            		unset( $result_body['results'][$a] );
            	}
            }
            */
        } else {
    ...
    )
    {
        ...
        if ( $check_value_enable == '1' && get_post_meta( $id, '_mpt_value_key', true ) != '1' ) {
            $log->error( 'Post box not check', array(
                'post' => $id,
            ) );
            return false;
        }
